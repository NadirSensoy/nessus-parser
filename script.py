...
try:
    ...
    all_ports = set()  # TÜM portları burada toplayacağız
    ...
    for line in lines:
        ...
        if match:
            ...
            all_ports.add(port)  # Tüm port değerlerini topluyoruz
        ...
        if match:
            ...
            port = match.group(2)
            ip_port_dict[ip] = port
            all_ports.add(port)  # Tüm port değerlerini toplıyoruz
            filtered_lines.append(f"{ip}:{port}")
            continue
    ...
    # Portları sırala (all_ports kümesinden)
    sorted_ports = sorted(all_ports, key=int)
